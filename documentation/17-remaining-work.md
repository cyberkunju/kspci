...
| 🟠 | **The general-web tier needs one API key** | `sources.web_search` is written, wired and dark. Without it the engine searches the press, not the internet: no forums, no complaint boards, no blogs. Every keyless route was tried and each fails on robots or on datacenter blocking — the list is in the module docstring and in [18 §6](./18-engine-techniques.md). Set `SERPER_API_KEY` (or adapt the adapter to Brave/Tavily, ~10 lines) and the tier switches on. This is the single largest remaining gap in coverage. |
