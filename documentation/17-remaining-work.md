...
| 🔴 | **The render tier is now justified and still unbuilt** | One Playwright container: navigate, wait for network idle, hand the HTML to the trafilatura we already run. Apache-2.0 throughout, a few hundred lines — we fetch 48 urls a run, not 48,000, so none of Firecrawl's queueing or proxy tiering is warranted. The evidence is in: MSN carried the correct story three times and returned no text each time, and `/health` now reports which publishers an instance has learned it cannot read. Groundwork done (`verdict.py`); the browser itself is the work. See [18](./18-engine-techniques.md). |
...
| 🟠 | **Three on-site adapters fail in practice** | Every live run reported `onsite:livelaw`, `onsite:indiankanoon` and sometimes `onsite:thehindu` as failed. Each is one lost source tier per run, and the run says so, but they were never diagnosed — likely a changed search path or bot blocking. Worth a look, because two of the three are the *court* sources. |
