...
| | Item | Notes |
|---|---|---|
| 🟠 | **`deep` mode has never been run against live sources** | 300 s / 80 fetches / two rounds. The budget exists and the code path is exercised only by the offline suite. Cost and behaviour under it are unmeasured. |
